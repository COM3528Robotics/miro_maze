	...
	def thread_dispatch_proc(self):
		...
		try:
			...
			while self.is_active():
				...
				# if active
				if self.callbacks.cameras:

					# get camera frames
					caml_new = self.cameras_dispatch[0]
					camr_new = self.cameras_dispatch[1]

					# if a new frame has arrived
					if caml_new:

						# clear it
						self.cameras_dispatch[0] = None

						# if we have a previous frame, and an opposing frame
						if caml and camr:

							# get time differences between available frames
							t1 = abs(caml.timestamp - camr.timestamp)
							t2 = abs(caml_new.timestamp - camr.timestamp)

							# if old one is closer
							if t1 < t2:

								# dispatch
								self.callbacks.cameras([caml, camr])

							else:

								# dispatch
								self.callbacks.cameras([caml_new, camr])

								# clear so we don't historize it
								caml_new = None
					...
					if camr_new:
						...
						# clear it
						self.cameras_dispatch[1] = None

						# if we have a previous frame, and an opposing frame
						if caml and camr:

							# get time differences between available frames
							t1 = abs(camr.timestamp - caml.timestamp)
							t2 = abs(camr_new.timestamp - caml.timestamp)
							...
							if t1 < t2:
								...
								# dispatch
								self.callbacks.cameras([caml, camr_new])
